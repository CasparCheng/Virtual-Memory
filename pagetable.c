...
int allocate_frame(pgtbl_entry_t *p) {
	...
	if(frame == -1) { // Didn't find a free page.
		...
		/* Write victim page to swap if needed */
		/* Check if dirty-bit of page is set */
		if (coremap[frame].pte->frame & PG_DIRTY) {
			/* Write page to swap only if it's dirty */
			...
			/* Clear dirty-bit of page */
			coremap[frame].pte->frame &= ~PG_DIRTY;
			/* Set onswap-bit of page */
...
char *find_physpage(addr_t vaddr, char type) {
	...
	/* Check if pagetable is initialized */
	/* Initialize pagetable when
	   pde pointer is null or pde pointer's lowest bit is unset
	*/
	...
	if (p->frame & PG_VALID) {
		...
		if (p->frame & PG_ONSWAP) {
			...
			if (swap_pagein(frame, p->swap_off) != 0) {
				perror("Failed to read page data from swap");
				exit(1);
			}
